...
RunDP <- function(analysis_type, datpath, cndatafiles = NA, co_cluster_cna = F, sample.snvs.only = T, remove.snvs = F ,generate_cluster_ordering = F,
  ...
  if (analysis_type == 'nd_dp') {
    ...
    if(num.timepoints>1){
      ...
      mutCount[all.phasing.index[,1],] =  mut_single_1
      mutCount[all.phasing.index[,2],] =  mut_single_2
      WTCount[all.phasing.index[,1],] = wt_single_1
      WTCount[all.phasing.index[,2],] = wt_single_2
    ...
    GS.data = list(S.i=assign_result$S.i, V.h=assign_result$V.h, Pi.h=assign_result$Pi.h,mutBurdens=assign_result$mutBurdens, 
                   alpha=assign_result$alpha, y1=assign_result$mutCount, N1=assign_result$mutCount+assign_result$WTCount,
    ...
    tree.size.iter = sapply(1:no.iters,function(t){dim(whole.tree[[tree.output[t]]])[1]+1})
    tree.size.final = tree.size.iter[no.iters]
    converged.iter = which(tree.size.iter==tree.size.final)
    no.iters.burn.in = min(converged.iter)
    most.fre.tree.index = sort(table(tree.size.iter[converged.iter]),decreasing=TRUE)[1]
    most.fre.tree = whole.tree[[most.fre.tree.index]]
    
    write.table(paste0(output_folder,samplename,"_most_frequent_tree.txt"),sep="\t",row.names=F,quote=F)
