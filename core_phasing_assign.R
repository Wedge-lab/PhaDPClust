PhasingAssignment(mutCount=dataset$mutCount, 
  ...
  for(m in 2:2){
    ...
    for(t in 1:num.timepoints){
      Pi.h[m,,t] = infer.pi.h(S.i, t, m, mutCount,countsPerCopyNum, tree.structure)
    }
    ...
    #drop cluster
    if(m>800){
      drop.result = drop.cluster(S.i, m, clu.drop, iter.count, killclu)
    ...
    if(num.timepoints>2){
      find.tree = find.tree.multiple(m,
                                     V.h,
                                     Pi.h,
                                     S.i,
                                     C,
    ...
    tree.structure = find.tree[[1]]
    phy.clu.iter[,,m] = find.tree[[2]]
