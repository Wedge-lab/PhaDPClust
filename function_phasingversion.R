...
phasing.assign.onebyone = function(mut1.index, mut2.index,
  ...
  for(t in 1:num.timepoints){
      ...
      for (c in 1:C){
        ...
        if(!same.chr){
          pha.likeli[c] = 0.5*pha.likeli[c]+infer.likelihood.same(Pi.h = Pi.h,m = m-1,t=t,
                                                                         error.rate = error.rate,
                                                                         phy = 3,
                                                                         cellularity=cellularity,
                                                                         mut1.cluster = c,
  ...
  for(t in 1:num.timepoints){
    ...
    for (c in 1:C){
      ...
      pha.likeli[c] = infer.likelihood.same(Pi.h = Pi.h,m = m,t=t,
                                                   error.rate = error.rate,
                                                   phy = j,
                                                   cellularity=cellularity,
                                                   mut1.cluster = c,
                                                   ...
                                                   copy.1 = cp.1, copy.2 = cp.2, copy.total = cp.total)   
      if(!same.chr){
        pha.likeli[c] = 0.5*pha.likeli[c]+infer.likelihood.same(Pi.h = Pi.h,m = m,t=t,
                                                                       error.rate = error.rate,
                                                                       phy = 3,
                                                                       cellularity=cellularity,
                                                                       mut1.cluster = c,
  ...
  Pr.S[mut1.index,] = Pr.S[mut1.index,] / sum(Pr.S[mut1.index,])
  Pr.S[mut1.index,clu.drop] = 0
  pha.2.ass = which(rmultinom(1,1,Pr.S[mut1.index,])==1)  
  ...
  for(t in 1:num.timepoints){
    ...
    for(c.1 in 1:C){
      for(c.2 in 1:C){
        ...
        if(!same.chr){
          phalike.together = 0.5*phalike.together+infer.likelihood.same(Pi.h = Pi.h,m = m,t=t,
                                                                               cellularity=cellularity,
                                                                               error.rate =error.rate,
                                                                               phy = 3,
                                                                               mut1.cluster = c.1,
...
########################################
#drop cluster
drop.cluster = function(S.i, m, clu.drop, iter.count, killclu ){
...
########################################
#infer Pi.h
infer.pi.h = function(S.i,t, m, mutCount,countsPerCopyNum, tree.structure){
  if(m>500 & num.timepoints == 1){
    ...
    while(stru.constrain){
      ...
      if(length(which(is.na(Pi.h[m,,]))>0)){
        stru.constrain=1
      }
...
find.tree.single= function(m,
  ...
  tree.temp = grow.tree(CCF.order,Pi.h,C,num.timepoints,soft.threshold=0)  
...
{ 
  ...
  for(i.row in 1:dim(indi.matrix)[1]){
    ...
    tree.temp = grow.tree(CCFs.sort,Pi.h,C,num.timepoints,soft.threshold = -0.05)
    tree.str.temp = tree.temp$Tree
    tree.phy.temp = tree.temp$Phy
...
select.tree.likelihood = function(m, V.h, Pi.h, S.i, num.timepoints,
                                     used.clu,
                                     all.phasing.index,all.phasing.info,all.phasing.reads,
                                     tree.str.temp, tree.phy.temp,sample.tree=F,error.rate)
{
  vh.temp = rep(NA,C)
  ...
  for(i.pair in 1:phasing.pair.num){
    ...
    fre.pair.temp[i.pair,non.empty,1] = sapply(non.empty,function(c.1){exp(vh.temp[c.1])})
    fre.pair.temp[i.pair,non.empty,2] = sapply(non.empty,function(c.2){exp(vh.temp[c.2])})
    
    for(t in 1:num.timepoints){
      
      phasing.read = all.phasing.reads[i.pair,1:4,t]
      fre.pair.temp[i.pair,non.empty,1] = fre.pair.temp[i.pair,non.empty,1]*sapply(non.empty,function(c.1){exp(mutCount[mut1.index,t]*log(mutBurdens[c.1,t,mut1.index])+
                                                                                  WTCount[mut1.index,t]*log(1-mutBurdens[c.1,t,mut1.index])+vh.temp[c.1])})
      fre.pair.temp[i.pair,non.empty,2] = fre.pair.temp[i.pair,non.empty,1]*sapply(non.empty,function(c.2){exp(mutCount[mut2.index,t]*log(mutBurdens[c.2,t,mut1.index])+
                                                                                WTCount[mut2.index,t]*log(1-mutBurdens[c.2,t,mut1.index])+vh.temp[c.2])})    
      
      for(c.1 in used.clu){
        for(phy.iter in 1:4){
            phasing.pair.temp[i.pair,c.1,used.clu,phy.iter,t] = sapply(non.empty,function(c.2){infer.likelihood.same(Pi.h, m,t,phy = phy.iter,cellularity,
                                                                                                                     mut1.cluster=c.1,mut2.cluster=c.2,mut.index.input,phasing.read,
                                                                                                                     copy.1=cp.1,copy.2=cp.2,copy.total=cp.total,error.rate)})
        }
        if(!same.chr){
          phasing.pair.temp[i.pair,c.1,used.clu,phy.iter,t]=0.5*phasing.pair.temp[i.pair,c.1,non.empty,phy.iter,t]+0.5*phasing.pair.temp[i.pair,c.1,non.empty,3,t]        
        }
      }
    }    
  }
  ...
  for(i.tree in 1:num.tree){
    ...
    tree.pair.temp = sapply(1:phasing.pair.num,function(i){ log(sum(fre.pair.temp[i,non.empty,1]%*%t(fre.pair.temp[i,non.empty,2])* phasing.cal.temp[i,non.empty,non.empty]))})    
...
grow.tree = function(CCF.order,Pi.h,C,num.timepoints,soft.threshold){
  ...
  for(clu.index in 3:length(CCF.order)){
    ...
    for(tree.used in tree.grow){
      ...
      for (ance in clu.tree){
        ...
        tree.grow.temp = c(tree.grow.temp,list(tree.new))
      }
    }
    tree.grow = tree.grow.temp
    tree.grow.temp = c()
  }
  
  if(length(tree.grow)==0){
    return(list())
  }
  
  phy.tree.infer = array(NA,dim=c(C,C,length(tree.grow)))
  modify.tree.iter = array(NA,dim = c(dim(tree.start),length(tree.grow)))
  for(i in 1:length(tree.grow)){
